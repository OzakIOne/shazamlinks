...
// @updateURL   https://github.com/OzakIOne/shazamlinks/blob/master/index.js
// ==/UserScript==
...
(function() {
  setTimeout(() => {
    ...
    const youtube = createElement('Youtube', `https://www.youtube.com/results?search_query=${title.innerText}+${artist.innerText}`);
    const spotify = createElement('Spotify', `https://open.spotify.com/search/${title.innerText} ${artist.innerText}`);
    const deezer = createElement('Deezer', `https://www.deezer.com/search/${title.innerText} ${artist.innerText}`);

    const container = document.querySelector('.flex-reset.btn.share.popup-btn.popup-inline').parentNode;
